  ...
  return (
    <div className="booking-shell">
      ...
      <div className="booking-card">
        ...
        {step === 2 && <div className="booking-step"><StepTitle number="02" title="Elegí día y horario" subtitle={`Duración aproximada: ${selectedService?.estimatedDurationHours ?? 3} horas.`} /><NativeDateField className="field" label="Fecha" value={data.date} min={today} onChange={handleDate} />{errors.date && <ErrorText>{errors.date}</ErrorText>}<fieldset className="time-field"><legend>Horarios disponibles</legend>{!data.date && <p className="empty-times" role="status">Seleccioná una fecha para ver los horarios disponibles.</p>}{loadingTimes && <p className="empty-times" role="status">Consultando horarios…</p>}<div className="time-options">{availableTimes.map((time) => <button type="button" className={data.time === time ? "selected" : ""} onClick={() => update("time", time)} key={time}>{time}</button>)}</div>{data.date && !loadingTimes && !errors.availability && availableTimes.length === 0 && <p className="empty-times" role="status">No hay horarios disponibles para este día.</p>}</fieldset>{errors.availability && <ErrorText>{errors.availability}</ErrorText>}{errors.time && <ErrorText>{errors.time}</ErrorText>}<p className="availability-note">Horarios consultados en la agenda real. Se comprueban nuevamente al confirmar.</p></div>}
